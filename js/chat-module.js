window.ChatModule = (() => {
    ...
    const AI_API_URL = (window.SITE_CONFIG && window.SITE_CONFIG.AI_API_BASE_URL) || "https://api.xn--efv066biyh.online";
    ...
    function createButtonNearExpert() {
        const actionBox = document.querySelector("#mainActionBox") || document.querySelector("#expertActionBox");

        if (!actionBox) {
            console.warn("ChatModule 找不到按钮挂载容器");
            return;
        }
